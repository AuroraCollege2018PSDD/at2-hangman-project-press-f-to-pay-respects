...
wordlist = ['kiwi', 'chicken', 'crow', 'stork', 'pigeon', 'swan', 'duck', 'peacock', 'parrot', 'sparrow', 'toucan', 'penguin', 'owl', 'hummingbird', 'seagull', 'ibis', 'emu', 'kingfisher', 'kookaburra', 'dove', 'conary', 'puffin', 'albatross', 'eagle']
lose = "You lose!!"
winText = "You win!!"
                ...
                if a.rectangle.collidepoint(mousePosition): #is the mouse click inside the letters rectangle
                    ...
                    for v in wordlistArray: #check whether that letter is in the wordlist
                        if a.text == v.text: #compare text of clicked letter to the word's text
                            ...
                            correctGuesses += 1
                            if correctGuesses == len(word): #see if word is guessed
                                guessesRemaining = 0
                                win = True
